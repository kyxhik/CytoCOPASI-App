	...
	public class SensTask extends AbstractTask {
		...
		public void run(TaskMonitor taskMonitor) throws Exception {
			...
			try {
				...
				problem.setSubTaskType(CSensProblem.SteadyState);
					...
					for (int j= 0; j<nodenumber; j++) { 
						if (AttributeUtil.get(currentNetwork, nodes.get(j), "display name", String.class).equals(model.getMetabolite(i).getObjectDisplayName())==true) {
							...
							if (sensCoeffs[i]>0.5) {
								...
								AttributeUtil.set(currentNetwork,  nodes.get(j), "Sensitivity Coefficient for Mapping",1000*Math.abs(sensCoeffs[i]), Double.class);
							}
