...
public class NewReactionToImportedModels {
	...
	public void addReaction(String quantityUnit, String timeUnit, CyNetwork copasiNetwork, CDataObject object, ObjectStdVector changedObjects) {
		...
		JLabel fluxLabel = new JLabel("Flux (" + quantityUnit+"/"+timeUnit+ ")");
					...
					if(reactionPane.getValue().equals(reactionOptions[0])) {
						...
						if (paramVals == null) {
							...
							if (String.valueOf(metabSplit[i].charAt(0)).matches("[-]?[0-9]+")) {
								...
								System.out.println(i+"metab:"+metabolites[i]);
								System.out.println("number of metabs at this point"+model.getNumMetabs());
								System.out.println("Are you finding metabs?"+model.findMetabByName(metabolites[i]));
							}
							...
							if(model.findMetabByName(metabolites[i])==null){
								...
								if(reactantProduct[0].contains(metabolites[i]) == true) {
									...
									if(reactantProduct[0].contains(metabolites[i]) == true) {
										CyNode reactantNode = newNetwork.createSpeciesNodeForSBML(copasiNetwork, metabolites[i], "species", newMetab.getKey(), newMetab.getCN().getString(), newMetab.getObjectDisplayName(),"cytosol", 1.0, "Reactions");

									}else if (reactantProduct[1].contains(metabolites[i]) == true) {
										CyNode productNode = newNetwork.createSpeciesNodeForSBML(copasiNetwork, metabolites[i], "species", newMetab.getKey(), newMetab.getCN().getString(), newMetab.getObjectDisplayName(),"cytosol", 1.0, "Reactions");
									}else if (reactantProduct.length == 3) {
										CyNode modifierNode = newNetwork.createSpeciesNodeForSBML(copasiNetwork, metabolites[i], "species", newMetab.getKey(), newMetab.getCN().getString(), newMetab.getObjectDisplayName(),"cytosol", 1.0, "Reactions");
									}
								}
							}
								}
								...
								if (chemEqString.contains("=") == true) {
									...
									reactionNode = newNetwork.createReactionsNode(copasiNetwork, reactionName.getText(), "reaction rev", reaction.getKey(), reaction.getCN().getString(), reaction.getObjectName(), true, reaction.getReactionScheme(), reaction.getFunction().getObjectName(), reaction.getFunction().getInfix(), subStr, subUni, proStr, proUni, modStr, modUni, parStr, parLabStr, parValStr);
										...
										reactionNode = newNetwork.createReactionsNodeForSBML(copasiNetwork, reactionName.getText(), "reaction rev", reaction.getKey(), reaction.getCN().getString(), reaction.getObjectName(), true, reaction.getReactionScheme(), reaction.getFunction().getObjectName(), reaction.getFunction().getInfix(), subStr, subUni, proStr, proUni, modStr, modUni, parStr, parLabStr, parValStr);
									...
									reactionNode = newNetwork.createReactionsNode(copasiNetwork, reactionName.getText(), "reaction irrev", reaction.getKey(), reaction.getCN().getString(), reaction.getObjectName(), false, reaction.getReactionScheme(), reaction.getFunction().getObjectName(), reaction.getFunction().getInfix(), subStr, subUni, proStr, proUni, modStr, modUni, parStr, parLabStr, parValStr);
										...
										reactionNode = newNetwork.createReactionsNodeForSBML(copasiNetwork, reactionName.getText(), "reaction irrev", reaction.getKey(), reaction.getCN().getString(), reaction.getObjectName(), false, reaction.getReactionScheme(), reaction.getFunction().getObjectName(), reaction.getFunction().getInfix(), subStr, subUni, proStr, proUni, modStr, modUni, parStr, parLabStr, parValStr);
						...
						for (int i = 0; i<nodeList.size(); i++) {
							...
							if (myNetwork.getAdjacentEdgeList(specNode, CyEdge.Type.ANY).size()==0) {
								...
								for (int j=0; j< model.getNumMetabs(); j++) {
									if (model.getMetabolite(j).getObjectDisplayName().equals(speciesName)==true) {
